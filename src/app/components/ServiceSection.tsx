...
const icons = [
  ...
  { icon: Settings, color: "bg-[var(--color-yellow)]/10 text-[var(--color-yellow)]", label: "Développement ERP/USSD", desc: "ERP sur-mesure, solutions USSD, intégration applicative, déploiement cloud." },
  { icon: FileCheck2, color: "bg-[var(--color-mint)]/10 text-[var(--color-mint)]", label: "Audit & Conseil SI", desc: "Audit de sécurité, conseil en transformation digitale, optimisation, conformité RGPD." },
...
export default function ServiceSection() {
  return (
    <section className="relative w-full px-0 py-16 md:py-32 flex flex-col items-center overflow-hidden">
      ...
      >
        Découvrez l&apos;expertise de CEIPI à travers une offre complète : digitalisation, ERP/USSD, audit & conseil SI, formation & certification ISO.
      </motion.p>
      <div className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-8 mb-10 md:mb-16 z-10">
        {icons.map((item, i) => {
          ...
          return (
            ...
            >
              <div className={`w-14 h-14 md:w-20 md:h-20 flex items-center justify-center rounded-full mb-3 md:mb-4 text-2xl md:text-3xl shadow-md transition-colors duration-300 group-hover:bg-[var(--color-mint)]/20 group-hover:text-[var(--color-navy)] ${item.color}`}>
